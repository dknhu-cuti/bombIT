...
class Agent:
    ...
    def _get_action(self, obs):
        ...
        # Late game: only 1 enemy left → hunt aggressively
        is_late_game = enemies_alive <= 1
        # Well armed: have enough firepower to be aggressive
        ...
        if self.escape_mode or my_pos in danger_now or my_pos in danger_soon:
            ...
            # Fallback: avoid immediate death
            loose = [
        ...
        # ── PRIORITY 2: ITEMS ─────────────────────────────────────────────
        # FIX 5: increased item search radius from 5 to 7
        ...
        # ── PRIORITY 3: BOMB IF CAN HIT ENEMY ────────────────────────────
        ...
        # ── PRIORITY 4 (FIX 1+2): HUNT ENEMY — late game or well armed ───
        # In late game (≤1 enemy) or when we have lots of firepower,
        # actively seek bombing positions instead of farming boxes.
        if (is_late_game or is_well_armed) and enemies and int(bombs_left) > 0:
        ...
        # ── PRIORITY 5: BOMB BOXES (FIX 3: smarter threshold) ───────────
        if bombs_left > 0 and my_pos not in bomb_positions:
            boxes_hit = self._count_boxes_in_blast(grid, my_pos, bomb_radius)
            # Require ≥2 boxes unless well-armed (then 1 box is ok)
        ...
        if box_spots:
            ...
            # FIX 4: relaxed path if safe path unavailable (go through danger_soon)
            if move is None:
        ...
        # ── PRIORITY 7: HUNT ENEMIES (early/mid game fallback) ───────────
    ...
    # ── HELPER: HUNT ENEMY (FIX 2) ────────────────────────────────────────
    def _hunt_enemy_action(self, grid, my_pos, enemies, blocked, danger_soon, danger_now,
                           bomb_radius, bombs_left):
        """
        Find the nearest position that allows bombing an enemy,
        move there and place bomb. Returns action or None.
        """
        if not enemies or bombs_left <= 0:
            return None

        # Build set of all cells from which we can line-hit at least one enemy
        kill_spots = set()
        ...
        if not kill_spots:
            # No line-of-sight spots found — just move toward the nearest enemy
        ...
        if my_pos in kill_spots:
            if self._can_escape_after_placing(
                ...
                self.escape_mode = True
                return 5
            # Can't escape here, try to reach a different kill spot
            kill_spots.discard(my_pos)

        # Move toward nearest kill spot (safe path first, relaxed fallback)
        move = self._move_to_targets(grid, my_pos, kill_spots, blocked, danger_soon)
        if move is None:
            move = self._move_to_targets_relaxed(
                grid, my_pos, kill_spots, blocked, danger_soon, danger_now
            )
        return move

    # ── HELPER: RELAXED PATHFINDING (FIX 4) ───────────────────────────────
    def _move_to_targets_relaxed(self, grid, start, targets, blocked, danger_soon, danger_now):
        """
        BFS toward targets allowing paths through danger_soon (timer > 1),
        but never stepping into danger_now (about to explode this tick).
        Used as fallback when no purely safe path exists.
        """
        if not targets:
            return None
        q = deque([(start, None)])
        ...
        while q:
            ...
            for a in [1, 2, 3, 4]:
                ...
                if npos in blocked and npos not in targets:
                    continue
                if npos in danger_now:  # Only block immediate explosions
                    continue
                seen.add(npos)
