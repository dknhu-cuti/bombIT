...
class Agent:
    ...
    def __init__(self, agent_id: int):
        ...
        model_path = Path(__file__).parent / "checkpoints" / "stage_C_final.zip"
        # Fallback: try stage B, then stage A if C is not found
        if not model_path.exists():
            model_path = Path(__file__).parent / "checkpoints" / "stage_B_final.zip"
        if not model_path.exists():
            model_path = Path(__file__).parent / "checkpoints" / "stage_A_final.zip"
